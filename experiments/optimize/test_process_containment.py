...
class RealWindowsTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="fast-containment-")
...
"""
    ...
    def test_owner_abrupt_exit_and_watchdog_leave_no_descendant(self):
        # Outer test timeout bounds this test even if the implementation is bad.
        module_dir = str(Path(__file__).resolve().parent)
        for watchdog in (False, True):
            with self.subTest(watchdog=watchdog):
                marker = self.directory/str(watchdog)
                ready = self.directory/(str(watchdog)+"-ready")
                child = ("import pathlib,time; pathlib.Path("+repr(str(ready))+").write_text('ready'); "
                ...
                source = ("import os,sys,time,tempfile,pathlib; sys.path.insert(0,"+repr(module_dir)+"); "
                          "from process_containment import WindowsJobProcess; from fast_regression import Budget; "
                          "p=WindowsJobProcess(); b=Budget(1,watchdog="+repr(watchdog)+"); b.containment=p; "
                          ...
                          "exec('while not ready.exists(): time.sleep(.005)'); "
                          +("time.sleep(10)" if watchdog else "os._exit(17)"))
                result = subprocess.run([sys.executable, "-c", source], capture_output=True, timeout=4)
                self.assertEqual(result.returncode, 124 if watchdog else 17, result.stderr)
