#!/usr/bin/env python3
"""Build only the new benchmark driver against the accepted existing runtime.
...
and build provenance are placed in build/configured-bench by default.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    parser.add_argument('--runtime-build', type=Path, default=ROOT/'build/native-compat')
    parser.add_argument('--output-dir', type=Path, default=ROOT/'build/configured-bench')
    ...
    source = ROOT/'experiments/optimize/configured_benchmark.cpp'
    ...
    binary = output/'optimize-configured-benchmark'
