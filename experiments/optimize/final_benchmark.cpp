/* Final three-cohort driver. Whole solve time includes every racing probe. */
...
struct Configuration {
  ...
  explicit Configuration(const std::string& route) {
    if (route == "auto-race") { automatic_race = true; return; }
  ...
  void print() const {
    if (automatic_race) {
      std::cout << "{\"entry_point\":\"solve_native_race\","
        "\"exploration_seconds\":2,\"probe_node_limit\":4096,"
        "\"candidates\":[\"structural_auto\",\"ordinary_native\"],"
        "\"presolve\":\"automatic\",\"components\":\"automatic\","
        "\"symmetry\":\"identical_columns\",\"cpu_warning\":true}";
      return;
    }
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (configuration.automatic_race) {
      O::NativeRaceOptions race; race.solve=options;
      race.exploration_seconds=2.0; race.probe_node_limit=4096;
      result=O::solve_native_race(snapshot,race);
    } else if (configuration.frontier) {
