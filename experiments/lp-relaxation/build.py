#!/usr/bin/env python3
"""Build pinned stock Gecode, optional LP Gecode, and focused test executables.
...
ROOT = HERE.parents[1]
BUILD = ROOT / "build/lp-relaxation"
...
def source_hashes(run):
    ...
    for name in paths.decode().split("\0"):
        if not name:
            continue
        relative = Path(name)
        if any(part in {"build", "results", "instances", "__pycache__"} for part in relative.parts):
            continue
        path = ROOT/relative
        if path.is_symlink():
...
def main():
    ...
    try:
        ...
        # Stock headers and generated configuration must win over the fallback
        # root include used solely for the shared, new lp-model.hpp builder.
        compile_binary("stock", HERE/"driver.cpp",
                       [args.stock_build, args.stock_source, ROOT], stock_libs)
        lp_includes = [args.modified_build, ROOT, args.highs_build, args.highs_source/"highs"]
        compile_binary("lp", HERE/"driver.cpp", lp_includes,
                       modified_libs+highs_libs, ["-DWITH_LP"])
        compile_binary("test-certificate", HERE/"tests/certificate.cpp", [ROOT], [])
        for name in ("backend", "propagator"):
            compile_binary("test-"+name, HERE/"tests"/(name+".cpp"),
                           lp_includes, modified_libs+highs_libs)
        metadata["binary_hashes"] = {name: file_hash(args.out/name) for name in
                                     ("stock", "lp", "test-certificate", "test-backend", "test-propagator")}
